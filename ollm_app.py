...
def get_open_ollm_model_ids():
    ...
    return open_ollm_model_ids

def download_model(open_ollm_model_id, local_files_only=False):
    """Download Open LLM and Llama models.

    Args:
        open_ollm_model_id (str): String of Open LLM model ID.
    ...
    if not local_files_only:
        print(f"Downloading {open_ollm_model_id}")
    try:
        snapshot_download(repo_id=open_ollm_model_id, local_files_only=local_files_only)
...
def get_model_and_tokenizer_class(open_ollm_model_id):
    ...
    """
    if ("open-calm" in open_ollm_model_id or
        "japanese-gpt-neox" in open_ollm_model_id or
        "stablelm" in open_ollm_model_id):
        model_class = AutoModelForCausalLM
        tokenizer_class = AutoTokenizer
    elif "llama" in open_ollm_model_id:
...
def create_prompt(chatbot, open_ollm_model_id, input_text_box):
    ...
    """
    if "instruction-sft" in open_ollm_model_id or "instruction-ppo" in open_ollm_model_id:
        sft_input_text = []
    ...
    elif "stablelm" in open_ollm_model_id:
...
def get_generate_kwargs(tokenizer, inputs, open_ollm_model_id, generate_params):
    """Get generate kwargs.
    ...
    if "stablelm" in open_ollm_model_id:
        stop = StopOnTokens()
...
def retreive_output_text(output_text, open_ollm_model_id):
...
def open_ollm_inference(chatbot, open_ollm_model_id, input_text_box, max_new_tokens, temperature, top_k, top_p, repetition_penalty):
    """Open LLM inference.
    ...
    dwonload_result = download_model(open_ollm_model_id, local_files_only=True)
    if dwonload_result != _DOWNLOAD_COMPLETED:
        return input_text_box, chatbot, dwonload_result
    
    model_class, tokenizer_class = get_model_and_tokenizer_class(open_ollm_model_id)
    
    print(f"Loading {open_ollm_model_id}")
    if (model_cache.get("preloaded_model_id") != open_ollm_model_id or 
        ...
        if platform.system() == "Darwin":
            model = model_class.from_pretrained(open_ollm_model_id, torch_dtype=torch.float32)
        else:
            model = model_class.from_pretrained(open_ollm_model_id, device_map="auto", torch_dtype=torch.float16)
        
        tokenizer = tokenizer_class.from_pretrained(
            open_ollm_model_id,
            use_fast=False if "japanese-gpt-neox" in open_ollm_model_id else True,
        )
        
        model_cache["preloaded_model_id"] = open_ollm_model_id
    ...
    prompt = create_prompt(chatbot, open_ollm_model_id, input_text_box)
    ...
    inputs = tokenizer(
        [prompt],
        return_tensors="pt",
        add_special_tokens=False if "japanese-gpt-neox" in open_ollm_model_id else True,
    ).to(model.device)

    t1 = time.time()
    with torch.no_grad():
        tokens = model.generate(
            **get_generate_kwargs(tokenizer, inputs, open_ollm_model_id, generate_params)
        )
    t2 = time.time()
    elapsed_time = t2-t1
    ...
    output = retreive_output_text(output, open_ollm_model_id)
...
def on_ui_tabs():
    open_ollm_model_ids = get_open_ollm_model_ids()

    block = gr.Blocks().queue()
    block.title = "Open LLM WebUI"
    with block as open_ollm_interface:
        with gr.Row():
        ...
        with gr.Row():
            ...
            with gr.Column():
                with gr.Row():
                    with gr.Column():
                        open_ollm_model_id = gr.Dropdown(label="LLM model ID", elem_id="open_ollm_model_id", choices=open_ollm_model_ids,
                                                         value=open_ollm_model_ids[2], show_label=True)
                    with gr.Column():
                        with gr.Row():
            ...
            download_model_btn.click(fn=download_model, inputs=[open_ollm_model_id], outputs=[status_text])
            
            generate_inputs = [chatbot, open_ollm_model_id, input_text_box, max_new_tokens, temperature, top_k, top_p, repetition_penalty]
            generate_btn.click(fn=user, inputs=[input_text_box, chatbot], outputs=[input_text_box, chatbot], queue=False).then(
                fn=open_ollm_inference, inputs=generate_inputs, outputs=[input_text_box, chatbot, status_text], queue=False)
    ...
    return [(open_ollm_interface, "Open LLM", "open_llm")]
