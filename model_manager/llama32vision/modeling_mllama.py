...
from ..base import ensure_tensor_on_device
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_inputs = ensure_tensor_on_device(layer_inputs, self.config.layer_device)
                decoder_layer.to(self.config.layer_device)
                layer_outputs = decoder_layer(**layer_inputs)
                decoder_layer.to("cpu")
                if idx == len(self.layers) - 1:
                    layer_outputs = ensure_tensor_on_device(layer_outputs, "cpu")
        ...
        all_self_attns = ensure_tensor_on_device(all_self_attns, "cpu")
...
class MllamaForConditionalGeneration(MllamaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if pixel_values is not None:
            ...
            vision_inputs = ensure_tensor_on_device(vision_inputs, self.config.layer_device)
            self.vision_model.to(self.config.layer_device)
            vision_outputs = self.vision_model(**vision_inputs)
            self.vision_model.to("cpu")
            vision_outputs = ensure_tensor_on_device(vision_outputs, "cpu")
