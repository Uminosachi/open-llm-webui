...
from transformers import (GenerationConfig, MllamaConfig, MllamaForConditionalGeneration,
                          MllamaImageProcessor, PreTrainedTokenizerFast)
...
def is_param_different_across_shards(key):
    ...
    """
    patterns = [r"vision_model.patch_embedding.weight", r"vision_model.(transformer|global_transformer).layers.(\d+).self_attn.(q|k|v|o)_proj.weight", r"vision_model.(transformer|global_transformer).layers.(\d+).mlp.fc1.(weight|bias)", r"vision_model.(transformer|global_transformer).layers.(\d+).mlp.fc2.weight",  r"multi_modal_projector.(weight|bias)",
...
def write_model(
    ...
    for key in all_keys:
        ...
        if ("cross_attention" in key or "text_model.layers" in key) and "language_model" in new_key:
            ...
            new_key = re.sub(r"layers.(\d+).", lambda _match, shift=shift: f"layers.{shift[int(_match.groups()[0])]}.", new_key)
